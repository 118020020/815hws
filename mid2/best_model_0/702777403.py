...
def train():
    ...
    num_epochs = 50  # Maximum number of epochs per fold
    ...
    data = load_data("midterm2_training_data.npy")
    labels = load_data("midterm2_training_labels.npy")
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(data)):
        ...
        logging.info(f"Data loaded for fold {fold + 1}. Train size: {len(train_dataset)}, Val size: {len(val_dataset)}")
...
def test(input_args):
    ...
    data = load_data(input_args.testing_data_path)
    ...
    np.save(f"{input_args.output_path}/{studentname_udid}.npy", predictions.cpu().numpy())
